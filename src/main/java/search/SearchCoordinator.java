...
public class SearchCoordinator implements OnRequestCallback {
    ...
    private List<String> readDocumentList() {
        ...
        return Arrays.asList(documentsDirectory.list()).stream()
                .map(documentsName->BOOKS_DIRECTORY+"/"+documentsName)
    ...
    private static List<List<String>> splitDocumentsList(int numberOfWorkers, List<String> documents){
        ...
        for(int i=0; i<numberOfWorkers;i++){
            int firstDocumentIndexExclusive=i*numberOfDocumentsPerWorker;
            int lastDocumentIndexExclusive=Math.min(firstDocumentIndexExclusive+numberOfDocumentsPerWorker, documents.size());
            if(firstDocumentIndexExclusive>= lastDocumentIndexExclusive){
                break;
            }
            List<String> currentWorkersDocuments = new ArrayList<>(documents.subList(firstDocumentIndexExclusive,lastDocumentIndexExclusive));
            workersDocuments.add(currentWorkersDocuments);
        }
    ...
    public List<Task>  createTasks(int numberOfWorkers, List<String> searchTerms){
        List<List<String>> workersDocuments= splitDocumentsList(numberOfWorkers,documents);
        List<Task>tasks=new ArrayList<>();
        for(List<String> documentsForWorkers : workersDocuments){
            Task task=new Task(searchTerms,documentsForWorkers);
            tasks.add(task);
        }
        return tasks;
    }

    public List<Result> sendTasksToWorkers(List<String> workers, List<Task> tasks){
    ...
    public byte[] handleRequest(byte[] requestPayload) {
        ...
        SearchModel.Response response= null;
        try {
    ...
    private SearchModel.Response createResponse(SearchModel.Request searchRequest) throws InterruptedException, KeeperException {
        ...
        if(workers.isEmpty()){
            System.out.println("No search workers available right now");
    ...
    private List<SearchModel.Response.DocumentStats> aggregateResults(List<Result> results, List<String> searchTerms) {
        ...
        System.out.println("Calculating scores for all documents");
        Map<Double, List<String>> scoreToDocuments= TFIDF.getDocumentsScores(searchTerms, allDocumentResults);
    ...
    private List<SearchModel.Response.DocumentStats> sortDocumentsByScore(Map<Double, List<String>> scoreToDocuments) {
        ...
        for(Map.Entry<Double, List<String>> docsScorePair : scoreToDocuments.entrySet()){
            double score = docsScorePair.getKey();
            for(String document : docsScorePair.getValue()){
